...
from pathfold.diffusion.diffusion_frame import Diffusion
...
class PathFoldFrameDiffusion(Diffusion):
	...
	def transform(self, batch):
		...
		try:
			coords, mask, single, pair, frame = batch
		except:
			coords, mask = batch
		coords = coords.float()
		mask = mask.float()
		...
		#update
		return T(rots, trans), mask, single, pair, frame
	...
	def p(self, ts, s, mask, noise_scale, single=None, pair=None, frame=None):

		# [b, 1, 1]
		w_noise = ((1. - self.alphas[s].to(self.device)) / self.sqrt_one_minus_alphas_cumprod[s].to(self.device)).view(-1, 1, 1)

		# [b, n_res]
		noise_pred_trans = ts.trans - self.model(ts, s, mask, single, pair, frame).trans
	...
	#update
	def loss_fn(self, tnoise, ts, s, mask, single=None, pair=None, frame=None):
